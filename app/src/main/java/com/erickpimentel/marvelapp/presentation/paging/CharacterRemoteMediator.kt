...
class CharacterRemoteMediator(
    ...
    private suspend fun getRemoteKeyForLastItem(
        ...
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.let { character ->
                Log.d("CharacterRemoteMediator", "Character ID for last item: ${character.name}")
                val remoteKeys = characterRemoteKeysDao.getRemoteKeys(id = character.id)
                Log.d("CharacterRemoteMediator", "Remote keys for last item: $remoteKeys")
                Log.d("CharacterRemoteMediator", "State pages: ${state.pages}")

                val nextPage = remoteKeys?.nextPage
                    ?: state.pages.lastOrNull { it.nextKey != null }?.nextKey

                Log.d("CharacterRemoteMediator", "nextPage: $nextPage")

                remoteKeys?.copy(nextPage = nextPage) ?: remoteKeys
            }
    }
